...
import { createDirection, deleteDirection, getAllDirections, updateDirection } from '@store/actions'
...
const directionsReducer = createReducer(initialState, builder => {
	builder.addCase(getAllDirections.pending, ( state ) => {
		...
		state.response = errorHandler(false)
	}).addCase(getAllDirections.rejected, ( state, action: PayloadAction<any, any> ) => {
		...
		state.status = 'FULFILLED'
		state.data = state.data ? [...state.data, action.payload.direction] : [action.payload.direction]
		state.response = errorHandler(false)
	}).addCase(createDirection.rejected, ( state, action: PayloadAction<any, any> ) => {
		...
		state.response = errorHandler(false)
	}).addCase(updateDirection.rejected, ( state, action: PayloadAction<any, any> ) => {
		state.status = 'REJECTED'
		state.response = errorHandler(true, action.payload.message)
		...
		state.status = 'FULFILLED'
		state.data = state.data?.filter(direction => direction.id !== action.payload.direction.id) || []
		state.response = errorHandler(false)
	}).addCase(deleteDirection.rejected, ( state, action: PayloadAction<any, any> ) => {
		state.status = 'REJECTED'
		state.response = errorHandler(true, action.payload.message)
	})
})
