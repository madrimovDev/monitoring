...
export const createDirection = createAsyncThunk('direction/create', async ( direction: NewDirection, { rejectWithValue } ) => {
	try {
...
export const updateDirection = createAsyncThunk('direction/update', async ( payload: PayloadType, { rejectWithValue } ) => {
	try {
		const result = await DirectionService.update(payload.id, payload.direction)
		return result.data
	} catch (e) {
		throw rejectWithValue(responseErrorMapper(e as AxiosError<{ message: string }>))
	}
...
export const deleteDirection = createAsyncThunk('direction/delete', async ( id: number, { rejectWithValue } ) => {
	try {
